...
@section('content')
    ...
    <div class="w-100">
        ...
        @endif
        <form id="editUserActionForm" name="" class="" method="POST" action="{{route('adminUserUpdate')}}">
            <input type="hidden" id="id" name="id" value="{{!empty($userInfo->id) ? intval($userInfo->id) : 0}}">
            @include('admin.adminUser.formFields')
        </form>
    </div>
    <script type="text/javascript">

        editUserActionFormACtion();

        function editUserActionFormACtion() {
            $('#editUserActionForm').validate({
                rules: {
                    user_name: "required",
